...
def load_mnist_trainval():
    ...
    #############################################################################
    img_num = np.shape(data)[0]
    train_cnt = int(img_num*0.8)
    train_data = data[:train_cnt]
    train_label = label[:train_cnt]
    val_data = data[train_cnt:]
    val_label = label[train_cnt:]
...
def generate_batched_data(data, label, batch_size=32, shuffle=False, seed=1024 ):
    '''
    ...
    batched_label = None
    if seed:
        #  seed was changed from None to 1024
        random.seed(seed)
    #############################################################################
    #
    ...
    # shuffle data and label at the same time if shuffle == True
    if shuffle == True:
        zipped = list(zip(data, label))
        random.shuffle(zipped)
        data, label = zip(*zipped)

    # create list of ndarrays (batched data and label)
    for i in range(0, len(data), batch_size):
        d_batch = np.array(data[i: i + batch_size])
        l_batch = np.array(label[i: i + batch_size])
        batched_data.append(d_batch)
        batched_label.append(l_batch)
...
def plot_curves(train_loss_history, train_acc_history, valid_loss_history, valid_acc_history):
    ...
    #############################################################################
    plt.figure()
    plt.plot(train_loss_history, label="training", color="blue")
    plt.plot(valid_loss_history, label="vaidation", color="red")
    plt.xlabel("Epochs")
    plt.ylabel("Loss")
    plt.grid()
    plt.title("Loss Curve")
    plt.legend()
    plt.savefig("Loss History.png", bbox_inches='tight')
    plt.close()

    plt.figure()
    plt.plot(train_acc_history, label="training", color="blue")
    plt.plot(valid_acc_history, label="vaidation", color="red")
    plt.xlabel("Epochs")
    plt.ylabel("Accuracy")
    plt.grid()
    plt.title("Accuracy Curve")
    plt.legend()
    plt.savefig("Accuracy History.png", bbox_inches='tight')
    plt.close()
    #############################################################################
    #                              END OF CODE                             #
    #############################################################################
